...
interface WishlistContextType {
  ...
  setActiveWishlist: (wishlist: Wishlist | null) => void;
  fetchWishlists: () => Promise<void>;
  fetchWishlistById?: (id: number) => Promise<Wishlist | null>;
  createNewWishlist: (
    name: string,
    description?: string,
    isPublic?: boolean
  ...
  deleteExistingWishlist: (id: number) => Promise<void>;
  isLoading: boolean;
  error: string | null;
}
...
export const WishlistProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const updateExistingWishlist = async (
    id: number,
    data: { name?: string; description?: string; isPublic?: boolean }
  ) => {
    if (!isAuthenticated) return;
    ...
    try {
      ...
      if (response?.data?.wishlist) {
        setWishlists((prev) =>
          prev.map((wl) => (wl.id === id ? response.data.wishlist : wl))
        );
  ...
  const deleteExistingWishlist = async (id: number) => {
    if (!isAuthenticated) return;
    ...
    try {
      ...
      if (activeWishlist?.id === id) {
        ...
        setActiveWishlist(
          remainingWishlists.length > 0 ? remainingWishlists[0] : null
        );
      }
    } catch (error: any) {
    ...
    try {
      ...
      const itemToRemove = wishlist.items.find(
        (item: ApiWishlistItem) =>
          item.product && item.product.id.toString() === productId.toString()
      );
      ...
      const removeResponse = await fetch(
        `${apiBaseUrl}/wishlists/${wishlist.id}/items/${itemToRemove.id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${user.token}`,
